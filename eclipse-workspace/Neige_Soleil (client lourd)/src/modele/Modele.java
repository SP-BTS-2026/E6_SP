...
public class Modele 
{
	...
	public static void insertAppartement(Appartement unAppartement) {
	    ...
	    // On liste les colonnes explicitement pour être ultra propre et éviter les erreurs
	    ...
	    executerRequete1(requete);
		 ...
		 /*****************Requete sur la table Materiel****************/
			...
			public static void insertReservation(Reservation r) {
			    // CORRECTION : On liste précisément les 6 colonnes qu'on injecte. 
			    // MariaDB mettra automatiquement l'ID en auto_increment et le statut à 'panier'.
			...
			public static void insertPersonnel(Personnel p) {
			    // CORRECTION : On liste les 4 colonnes réelles (sans email) pour éviter le décalage de structure
			...
			public static ArrayList<Personnel> selectAllPersonnel(String filtre) {
			    ...
			    try {
			        ...
			        while (rs.next()) {
			            // CORRECTION : rs.getString("email") supprimé. On envoie une chaîne vide "" 
			            // pour ne pas faire crasher le constructeur de ton objet Personnel
			...
			public static void updatePersonnel(Personnel p) {
			    // CORRECTION : Suppression de la mise à jour du champ email
			...
			public static String[] recupererDonneesFacture(int id_reser) {
			    ...
			    // Requête corrigée avec : type_appart et prix_hebdo
			    ...
			    try {
			        uneBdd.seConnecter();
			        java.sql.Statement unStat = uneBdd.getMaConnection().createStatement();
			        java.sql.ResultSet rs = unStat.executeQuery(requete);
